...
def prepare_model_settings(label_count, sample_rate, clip_duration_ms,
  ...
  window_stride_s = window_stride_ms / 1000
...
def create_conv_model(fingerprint_input, model_settings, is_training):
  ...
  'Convolutional Neural Networks for SOBmall-footprint Keyword Spotting' paper:
  ...
  max_pool = tf.nn.max_pool(first_dropout, [1, 2, 2, 1], [1, 2, 2, 1], 'VALID')
  ...
  #Calculate max absolute value of output signals, bias and weights
  first_conv_max=tf.reduce_max(tf.abs(first_conv))
  second_conv_max=tf.reduce_max(tf.abs(second_conv))
  first_bias_max=tf.reduce_max(tf.abs(first_bias))
  first_weights_max=tf.reduce_max(tf.abs(first_weights))
  second_bias_max=tf.reduce_max(tf.abs(second_bias))
  second_weights_max=tf.reduce_max(tf.abs(second_weights))
  final_fc_bias_max=tf.reduce_max(tf.abs(final_fc_bias))    
  final_fc_weights_max=tf.reduce_max(tf.abs(final_fc_weights))

  if is_training:
    return final_fc, dropout_prob , max_pool , first_conv_max, second_conv_max, first_bias_max, first_weights_max, second_bias_max, second_weights_max, final_fc_bias_max, final_fc_weights_max 
  else:
    return final_fc
